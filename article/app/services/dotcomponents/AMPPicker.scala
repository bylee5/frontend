...
object AMPPageChecks extends Logging {
  ...
  def hasOnlySupportedElements(page: PageWithStoryPackage, blocks: APIBlocks): Boolean = {
...
object AMPPicker {
  ...
  private[this] def ampFeatureWhitelist(page: PageWithStoryPackage, request: RequestHeader, blocks: APIBlocks): Map[String, Boolean] = {
    Map(
      ...
      ("hasOnlySupportedElements", AMPPageChecks.hasOnlySupportedElements(page, blocks)),
